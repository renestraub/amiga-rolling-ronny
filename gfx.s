...
OpenScreen:	movem.l	d1-d5/a0-a4,-(sp)
		...
		SYSCALL	AddTail				* Am Ende anhängen
...
.EndColMap:	lea	sc_Bitmap(A0),a1		* A1=BitmStr
...
.EndBitmStr:	lea	sc_CopperList(a0),a1		* A1=CopperList

		move.w	#CWAIT,(a1)+			* First Wait
		move.w	#-3,(a1)+			* Top of Screen
...
.LoRes:		and.w	#$00FC,d0
		...
		clr.w	(A1)+				* 2 Linien schwarz
							* jetzt Screen anzeigen
...
;		or.w	SystemBplcon0,d2		* GenLock !!!
		...
		move.w	#CWAIT,(A1)+			* Wait
		move.w	sc_Height(A0),(a1)		* Am ScrEnde aufhören
		add.w	#0,(a1)+
...
.LoRes:		move.w	#$0500+SCR_X_POS,d0
		move.w	#$4000+SCR_X_POS,d1		
...
*****************************************************************************
* Function	: InsertPalWait (internal)
* Parameters	: A3 = Copperliste
* Result	: none
*****************************************************************************

InsertPalWait:	tst.b	PalFlag
		bne.s	.End

		move.l	#$FFDFFFFE,(A3)+		* Insert a PALWAIT
	;	move.l	#$01800FFF,(A3)+		* Farbe Schwarz
		st.b	PalFlag
...
*****************************************************************************
* Function	: EndCopperlist (internal)
* Parameters	: A3 = Copperliste
* Result	: none
*****************************************************************************

EndCopperList:	bsr	InsertPalWait			* PalWait einfügen
							* (wenn noch keines gesetzt)

		move.l	#(MAXSCRHEIGHT-$100)*$1000000+$21FFFE,(A3)+

		move.l	#$01800000,(A3)+		* Farbe Schwarz
		move.l	#$01000200,(A3)+		* DMA aus
		move.l	#$FFFFFFFE,(A3)			* Das Wait das niemals kam
...
MergeCopper:	movem.l	d0-d7/a0-a6,-(sp)
...
.NextScreen:	move.l	(a0),a0				* Get next screen
		tst.l	(a0)				* No more screen available
		beq	.End				* -> End

		move.w	sc_TopEdge(A0),ScrYPos		* YStart
		lea	sc_CopperList(a0),a1		* CopperStructure


	******	Get StartPos of NextScreen (or Default) ******

		move.l	(a0),a2				* Next Screen
		tst.l	(a2)				* Check auf LastScreen
		bne.s	.MoreScrs

		move.w	#16384,NextScrYPos		* Wenn kein Screen mehr kommt
		bra.s	.CheckEquality			* geht dieser bis zum Schluss

.MoreScrs:	move.w	sc_TopEdge(a2),NextScrYPos	* Next YStart

.CheckEquality:	move.w	ScrYPos,d0
		cmp.w	NextScrYPos,d0			* Screen priority
		bge.s	.NextScreen



.GetCmd:	move.w	(a1)+,d0			* Get Command
...
.CMove:		move.w	d0,(A3)+			* Copy Command
...
.InsertWait:	move.w	(a1)+,d0
		add.w	ScrYPos,d0			* ScrPosition

		cmp.w	NextScrYPos,d0			* reached Next Screen ?
		bge.s	.NextScreen			* Yes ->


		add.w	#SCR_Y_POS,d0			* ViewOffset
...
.AbovePal:	cmp.w	#MAXSCRHEIGHT,d0		* Höhenbeschränkung
		bge.s	.End				* ausserhalb -> Ende

		moveq	#0,d1				* Build WaitCmd for Copper
		move.b	d0,d1
		asl.w	#8,d1
		or.b	#$21,d1
...
.End:		bsr	EndCopperList			* Copperliste abschliessen
...
OldMergeCopper:	movem.l	d0-d7/a0-a4,-(sp)
		lea	GfxBase,a4
		clr.b	d5				* PalFlag
		move.l	gfx_DrawCpr(A4),a3		* CopperListe
		lea	ScrList,a0
...
Move:		move	d0,wn_CursorX(A0)		* Set CrusorX
		move	d1,wn_CursorY(A0)		* Set CursorY
		rts
...
ReadPixel:	movem.l	d1-d4/a0-a1,-(sp)
...
WritePixel:	movem.l	d0-d3/a0-a1,-(sp)
...
GetData:	tst.w	d2				* Width = 0
		bgt.s	5$
		tst.w	d3				* Height = 0
		bgt.s	5$
		rts

5$:		movem.l	d0-d7/a0-a4,-(sp)
		move.l	a1,a4
...
OpenWindow:	movem.l	d1-d5/a0-a3,-(sp)
	...
	;	clr.w	wn_CursorY(A0)
		...
		SYSCALL	AllocMem			* Allocate SaveBuffer
...
.NoTitel:	btst	#WNB_BORDERLESS,wn_Flags+1(A0)
		bne.s	.NoBorder

		bsr.s	DrawWindowBorder

.NoBorder:	move.b	nw_APen(A3),wn_APen(A0)
		move.l	a0,d0
...
ClearWindow:	movem.l	d0-d6/a0-a1,-(sp)
...
DoLineFeed:	move.w	#2,wn_CursorX(A0)		* Increase YPos
...
OpenFont:	movem.l	d1-d2/a0-a3,-(SP)
;		bsr	CloseFont
		move.l	#fo_SIZEOF,d0
...
ZeichenSatz:	dc.b	0,0,0,0,0,0,0,0				; Space
		...
		dc.b	$1E,$33,$33,$33,$33,$1E,$00,$00		; 0
