...
MYEXEC:
		...
		XDEF	ReadPixel,WritePixel,Draw
		XDEF	RectFill,GetData,PutData
		XDEF	Text,RightText,MidText,Print,StrLen
		XDEF	OpenWindow,CloseWindow,ClearWindow
		XDEF	OpenScreen,CloseScreen,LoadRGB,MoveScreen
		XDEF	OpenFont,CloseFont
		XDEF	FadeOut,FadeIn
		XDEF	RasterDelay,WaitRaster
...
MoveMcr:	MACRO
		move	\1,wn_CursorX(A0)
		move	\2,wn_CursorY(A0)
		ENDM
...
;		move.w	ns_TopEdge(A1),D1		* TopEdge
;		move.l	(A0),d0				* NextScreen
;12$:		move.l	d0,a0
;		move.l	(A0),d0				* No more Screens
;		bra.s	13$				* --> EXIT
;		cmp.w	sc_TopEdge(A0),D1		* TopEdge
;		ble.s	12$				* Smaller
;		bgt.s	12$				* Smaller
;13$:		move.l	4(a0),d0
;		move.l	a1,4(a0)
...
.StdBitmStr:	move.w	sc_Width(a0),d0			* Width in Pixels
		...
		add.w	#64,d0				* CHEAT for ColorMap
		jsr	AllocClearMem			* D0=ScreenBase
...
CloseScreen:	movem.l	d0/a0-a1,-(sp)
		...
		jsr	FreeMem				* Free Planes
		move.l	a0,a1
		jsr	Remove				* Remove from ScrList
		jsr	FreeMem				* Free ScreenStruct
		bsr	MergeCopper
		movem.l	(sp)+,d0/a0-a1
		rts
...
MoveScreen:	movem.l	d0-d6/a0-a2,-(sp)
		...
		muls	bm_BytesPerRow(a2),d6		* D3=BitPlaneOffset
		move.l	#0,d6
		bra.s	.BottomBorder

.TopBorder:	moveq	#0,d6
		cmp.w	#320,d2
		blt.s	.BottomBorder
...
MergeCopper:	movem.l	d0-d7/a0-a4,-(sp)
		lea	GfxBase,a4
		clr.b	d5				* PalFlag
...
5$:		movem.l	d0-d7/a0-a4,-(sp)
		...
		clr.w	bltcon1(A5)
		clr.w	bltdmod(A5)			* Destination Modulo
...
CloseWindow:	movem.l	d0-d3/a0-a1,-(sp)
		...
		move.l	wn_SaveBack(A0),a1		* Buffer
		jsr	FreeMem				* Free SaveBuffer	

.NoBackRestore:	move.l	a0,a1
		jsr	FreeMem				* Free WindowStructure
		movem.l	(sp)+,d0-d3/a0-a1
		rts
...
OpenWindow:	movem.l	d1-d5/a0-a3,-(sp)
		...
		jsr	AllocMem			* Allocate SaveBuffer
...
CloseFont:	movem.l	d0/a0-a2,-(SP)
		...
		move.l	fo_Bob(a0),a1
		jsr	FreeMem				* Free BobStructure
		move.l	a0,a1
		jsr	FreeMem				* Free FontStructure
